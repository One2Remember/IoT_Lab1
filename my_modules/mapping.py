...
def init_environment():
    ...
    environment[0] = np.ones(ROOM_WIDTH)
    environment[-1] = np.ones(ROOM_WIDTH)
    environment[:,0] = np.ones(ROOM_HEIGHT)
    environment[:,-1] = np.ones(ROOM_HEIGHT)
    
    # set car initial heading to 0
    car_heading = 0 
    # set car initial location to top of grid, middle position
    car_location = np.array([CAR_HEIGHT, ROOM_WIDTH // 2])
...
def update_environment(readings):
    ...
    true_coords[0] < ROOM_HEIGHT and true_coords[1] >= 0 and 
    true_coords[1] < ROOM_WIDTH)]
    ...
    for i in range(true_coords[0].size() - 1):
        ...
        if coord_inbounds(true_coords[i+1]):
...
def set_neighborhood_around_obstacle(x, y):
    ...
    x_s = x_s[x_s >= 0 and x_s < ROOM_WIDTH]
    y_s = y_s[y_s >=0 and y_s < ROOM_HEIGHT]
